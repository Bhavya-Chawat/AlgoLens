  ...
  if (aiNodes && Array.isArray(aiNodes) && aiNodes.length > 0) {
    ...
    aiNodes.forEach(n => { 
      if (n.val !== null && n.val !== undefined && n.val !== '?') {
        nodeMap[n.id] = { ...n, left: null, right: null }; 
      }
    });
    aiNodes.forEach(n => {
      if (!nodeMap[n.id]) return;
      if (n.left !== null && n.left !== undefined && nodeMap[n.left]) nodeMap[n.id].left = nodeMap[n.left];
      if (n.right !== null && n.right !== undefined && nodeMap[n.right]) nodeMap[n.id].right = nodeMap[n.right];
    });
    // Assuming root is the first node or node 0
    root = nodeMap[0] || nodeMap[aiNodes[0].id] || Object.values(nodeMap)[0];
